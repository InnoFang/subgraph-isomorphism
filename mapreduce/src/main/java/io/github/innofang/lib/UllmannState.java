...
public class UllmannState extends State {
    ...
    public void addPair(Pair<Integer, Integer> pair) {
        ...
        for (int i = len; i < sourceVertexSize; i++) {
            M[i][targetVertex] = 0;
        }
        refine();
    ...
    public boolean isFailure() {
        ...
        if (sourceVertexSize > targetVertexSize) {
            return true;
        }

        boolean flag = false;
        for (int i = mappingSize; i < sourceVertexSize; ++i) {
            for (int j = 0; j < targetVertexSize; ++j) {
                if (M[i][j] != 0) {
                    flag = true;
                    break;
                }
    ...
    public PairIterator iterator() {
        return new UllmannStatePairIterator(mapping.size(), sourceVertexSize, targetVertexSize);
    }
